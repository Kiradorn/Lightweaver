    ...
    def quadrature(self, Nrays: Optional[int]=None,
        ...
        if self.Ndim == 1:
            ...
            if self.Ndim == 2:
                ...
                self.mux[:Nrays // 2,0] = np.sin(theta) * np.cos(chi)
                self.mux[Nrays // 2:,0] = -np.sin(theta) * np.cos(chi)
                self.mux[:,1] = - self.mux[:,0]
                self.mux = np.ascontiguousarray(self.mux)
                self.muz = np.zeros([Nrays,2])
                self.muz[:,0] = -np.tile(list(np.cos(theta)),2)
                self.muz[:,1] = np.tile(list(np.cos(theta)),2)
                self.muz = np.ascontiguousarray(self.muz)
                self.wmu = np.zeros([Nrays,2])
                self.wmu = np.ascontiguousarray(np.tile(np.tile(quad[:, 0],2),(2,1)).T)
                # self.wmu[Nrays // 2:,0] = quad[:, 0]
                # self.wmu[:,1] = self.wmu[:,0]
                # for i in range(2):
                #     self.wmu[:,i] /= np.sum(self.wmu[:,i])
